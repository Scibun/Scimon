...
use crate::regexp::regex_minify::MinifyRegExp;
...
impl PrimeDownMinify {
    ...
    pub fn js(code: &str) -> String {
        ...
        let code = Regex::new(
            &format!(r"\b({})\b", MinifyRegExp::MIN_JS_KEYWORDS)
        ).unwrap().replace_all(
            &code, " $1 "
        );
    ...
    pub fn css(code: &str) -> String {
        let css = Regex::new(MinifyRegExp::MIN_CSS_REMOVE_MULTI_LINE_COMMENT).unwrap().replace_all(code, "");
        let css = Regex::new(MinifyRegExp::MIN_CSS_REMOVE_WHITESPACE).unwrap().replace_all(&css, " ");
        let css = Regex::new(MinifyRegExp::MIN_CSS_REMOVE_SPACES).unwrap().replace_all(&css, "$1");
